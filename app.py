...
/* ========== WARNA KUNING UNTUK CARI JUDUL & FILTER KONTEN ========== */
.search-title-label, .filter-title-label {
...
/* HAPUS KEYBOARD DOUBLE ARROW RIGHT DI SIDEBAR */
...
# -----------------------------
# Sidebar - RAPIKAN PETAKANNYA
# -----------------------------
with st.sidebar:
    # Sidebar Header - DIURAPIKAN
    st.markdown(
        """
    ...
    # Navigation Menu - DIURAPIKAN
    st.markdown('<div style="margin-bottom: 1rem;"><h4 style="color: #E50914 !important; margin-bottom: 0.8rem; font-weight: 800; font-size: 1.1rem;">🧭 MENU NAVIGASI</h4></div>', unsafe_allow_html=True)
    
    # Radio buttons dengan styling yang lebih rapi
    st.markdown("""
    <style>
    .nav-radio-container {
    ...
    st.markdown('<div class="nav-radio-container">', unsafe_allow_html=True)
    page = st.radio(
        "",
    ...
    # Data Configuration - DIURAPIKAN
    st.markdown('<div style="margin: 1.2rem 0 0.8rem 0;"><h4 style="color: #E50914 !important; margin-bottom: 0.8rem; font-weight: 800; font-size: 1.1rem;">⚙️ KONFIGURASI DATA</h4></div>', unsafe_allow_html=True)
    ...
    # System Status - DIURAPIKAN
    st.divider()
    st.markdown('<div style="margin: 1.2rem 0 0.8rem 0;"><h4 style="color: #E50914 !important; margin-bottom: 0.8rem; font-weight: 800; font-size: 1.1rem;">📊 STATUS SISTEM</h4></div>', unsafe_allow_html=True)

    # Quick Filter - DIURAPIKAN
    st.markdown('<div style="margin: 1.2rem 0 0.8rem 0;"><h4 style="color: #E50914 !important; margin-bottom: 0.8rem; font-weight: 800; font-size: 1.1rem;">🔍 FILTER CEPAT</h4></div>', unsafe_allow_html=True)
...
# Sidebar status + stats - DIURAPIKAN
with st.sidebar:
    # Display system status - DIURAPIKAN
    if st.session_state.data_loaded:
        st.markdown(
            """
            <div style="background: linear-gradient(135deg, rgba(0, 184, 148, 0.15) 0%, rgba(0, 168, 132, 0.15) 100%);
                        border-radius: 10px; border: 1px solid rgba(0, 184, 148, 0.4);
                        padding: 1rem; margin-bottom: 1.2rem;">
                <div style="display: flex; align-items: center; gap: 0.8rem;">
                    <div style="font-size: 1.5rem; color: #00B894; flex-shrink: 0;">✅</div>
                    <div>
                        <div style="font-weight: 800; color: #00B894 !important; font-size: 0.95rem; line-height: 1.1;">SISTEM AKTIF</div>
    ...
    # Display dataset stats - DIURAPIKAN
    st.markdown(
        f"""
        <div style="background: rgba(15, 15, 15, 0.9); backdrop-filter: blur(15px);
                    border-radius: 12px; border: 1px solid rgba(229, 9, 20, 0.4);
                    padding: 1.2rem; margin-bottom: 1.5rem;">
            <div style="display: flex; justify-content: space-between; margin-bottom: 1.2rem;">
                <div>
                    <div style="font-size: 0.85rem; color: #FFFFFF !important; font-weight: 600; margin-bottom: 0.4rem;">TOTAL DATA</div>
                    <div style="font-size: 1.7rem; font-weight: 900; color: #E50914 !important; text-shadow: 1px 1px 0 #000000;">{stats['total']:,}</div>
                </div>
                <div style="text-align: right;">
                    <div style="font-size: 0.85rem; color: #FFFFFF !important; font-weight: 600; margin-bottom: 0.4rem;">JENIS UNIK</div>
                    <div style="font-size: 1.7rem; font-weight: 900; color: #E50914 !important; text-shadow: 1px 1px 0 #000000;">{stats['unique_types']}</div>
                </div>
            </div>
            <div style="display: flex; justify-content: space-between;">
                <div>
                    <div style="font-size: 0.85rem; color: #FFFFFF !important; font-weight: 600; margin-bottom: 0.4rem;">FILM</div>
                    <div style="font-size: 1.4rem; font-weight: 900; color: #00B894 !important; text-shadow: 1px 1px 0 #000000;">{stats['movies']:,}</div>
                </div>
                <div style="text-align: right;">
                    <div style="font-size: 0.85rem; color: #FFFFFF !important; font-weight: 600; margin-bottom: 0.4rem;">SERIAL TV</div>
                    <div style="font-size: 1.4rem; font-weight: 900; color: #FDCB6E !important; text-shadow: 1px 1px 0 #000000;">{stats['tv_shows']:,}</div>
                </div>
            </div>
        </div>
        """,
    ...
    # Quick Filter Slider - DIURAPIKAN
    min_year = stats.get("min_year", 1900)
    max_year = stats.get("max_year", datetime.now().year)
    
    st.markdown('<div style="margin-bottom: 0.5rem; color: #FFFFFF !important; font-weight: 600; font-size: 0.9rem;">📅 FILTER TAHUN RILIS</div>', unsafe_allow_html=True)
    sidebar_year_range = st.slider(
        "Pilih rentang tahun:", 
        min_value=min_year, 
        max_value=max_year, 
...
if page == "🎯 REKOMENDASI":
    ...
    with tabs[0]:
        ...
        with col1:
            ...
            # Filter Tipe Konten dengan WARNA KUNING
            st.markdown('<div class="glass-panel" style="padding: 1.5rem;">', unsafe_allow_html=True)
            
            # Filter Tipe Konten dengan label KUNING
            st.markdown('<p class="filter-title-label">🎭 FILTER TIPE KONTEN</p>', unsafe_allow_html=True)
            ...
            # Pilih Judul dengan WARNA KUNING
            st.markdown('<p class="search-title-label" style="margin-top: 1.2rem;">📝 CARI JUDUL</p>', unsafe_allow_html=True)
            ...
            st.markdown('<p class="input-label">Pilih rentang tahun rilis:</p>', unsafe_allow_html=True)
    ...
    with tabs[1]:
        ...
        # Input Pencarian dengan label custom KUNING
        col_search1, col_search2 = st.columns([3, 1])
        with col_search1:
            st.markdown('<p class="search-title-label">🔍 MASUKKAN KATA KUNCI</p>', unsafe_allow_html=True)
            query = st.text_input(
        ...
        # Filter Pencarian dengan label KUNING
        ...
        with col_filter1:
            st.markdown('<div class="stats-card">', unsafe_allow_html=True)
            st.markdown('<p class="filter-title-label" style="margin-bottom: 0.6rem; font-size: 0.9rem;">FILTER TIPE</p>', unsafe_allow_html=True)
...
# -----------------------------
    ...
    # Search input dengan label custom KUNING
    st.markdown('<p class="search-title-label">🔍 CARI DALAM DATASET</p>', unsafe_allow_html=True)
    ...
    # Slider dengan label custom KUNING
    st.markdown('<p class="filter-title-label" style="margin-top: 1rem;">📊 PILIH JUMLAH SAMPEL</p>', unsafe_allow_html=True)
