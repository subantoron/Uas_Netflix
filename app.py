...
/* ---------- Expander (FIX agar icon tidak nabrak teks) ---------- */
...
/* =========================================================
   SUPER IMPORTANT FIX:
   SELECTBOX STREAMLIT (BaseWeb select + popover dropdown)
   -> supaya teks dropdown "Pilih Judul" / "Pilih Konten" terlihat
   ========================================================= */

/* Control (kotak select) */
...
@media (max-width: 768px){
  ...
  .netflix-subtitle{ font-size:1.15rem !important; }
}
</style>
"""
...
def display_recommendation_card(r: pd.Series, rank: int) -> None:
    ...
    st.markdown(
        ...
        <div class="glass-panel" style="padding:1.2rem;">
          ...
          {"<div style='margin-top:0.8rem;'><strong style='color:var(--red);'>🎭 Genre:</strong> "
            + genre + "</div>" if genre else ""}
          ...
          <div style="margin-top:0.9rem; display:flex; gap:0.8rem; flex-wrap:wrap;">
            {"<div class='stats-card' style='flex:1; min-width:220px; padding:0.9rem;'><div style='color:var(--red); font-weight:900;'>🎬 Sutradara</div><div style='margin-top:0.45rem; font-weight:700;'>" + director + "</div></div>" if director else ""}
            {"<div class='stats-card' style='flex:1; min-width:220px; padding:0.9rem;'><div style='color:var(--red); font-weight:900;'>🌍 Negara</div><div style='margin-top:0.45rem; font-weight:700;'>" + country + "</div></div>" if country else ""}
          </div>
        </div>
        """,
        unsafe_allow_html=True,
...
with st.sidebar:
    st.markdown(
        ...
        <div class="stats-card" style="text-align:center;">
          ...
          <div style="color:var(--muted); font-weight:700;">Recommender System</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    use_local = st.checkbox("Gunakan dataset lokal (netflix_titles.csv)", value=True)
...
if page == "🎯 REKOMENDASI":
    ...
    with tabs[0]:
        ...
        with col1:
            ...
            # OPTIONAL: search judul biar lebih nyaman
                ...
                st.markdown(
                    ...
                    <div class="glass-panel" style="padding:1.2rem;">
                      ...
                      <div style="margin-top:0.8rem; color:var(--text); line-height:1.45;">
                        {_safe_str(item['description'])[:140]}{'...' if len(_safe_str(item['description'])) > 140 else ''}
                      </div>
                    </div>
                    """,
                    unsafe_allow_html=True,
        ...
        st.markdown(
            ...
            <div class="glass-panel">
              ...
              <p style="line-height:1.55;">
                ...
                Fitur teks diubah menjadi vektor menggunakan <b>TF‑IDF</b>, lalu kemiripan dihitung memakai
                <b>Cosine Similarity</b> (melalui <i>linear_kernel</i>).
              </p>
              <p style="line-height:1.55;">
                Output yang ditampilkan adalah Top‑N judul dengan skor kemiripan tertinggi.
              </p>
            </div>
            """,
            unsafe_allow_html=True,
